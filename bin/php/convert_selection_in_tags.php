...
function convertObject(eZContentObject $object)
{
    ...
    if (isset($dataMap[$options['selection_attribute']]) && $dataMap[$options['selection_attribute']]->hasContent()){
        /** @var \Opencontent\Opendata\Api\Values\Tag[] $tags */
        $names = explode('|', $dataMap[$options['selection_attribute']]->toString());
...
try
{
    ...
    if ($class instanceof eZContentClass){
        $selectionAttribute = getClassAttribute($class, $options['selection_attribute'], eZSelectionType::DATA_TYPE_STRING);
        ...
        do
        {
            ...
            foreach($items as $object){
                $i++;
                $cli->output($i . '/' . $count . " " . $object->attribute('name'));
