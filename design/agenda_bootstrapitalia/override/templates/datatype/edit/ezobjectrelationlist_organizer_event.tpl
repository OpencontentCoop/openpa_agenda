...
{if fetch(user, current_user).contentobject.class_identifier|eq('private_organization')}
    ...
    <p>{fetch(user, current_user).contentobject.name|wash()}</p>
    <input type="hidden"name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" value="{fetch(user, current_user).contentobject_id}"
