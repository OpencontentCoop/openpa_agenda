...
{else}
    ...
    {def $contacts = $attribute.content.relation_list}
    {if and(count($contacts)|eq(0), fetch(user, current_user).contentobject.class_identifier|eq('private_organization'))}
        {set $contacts = fetch(user, current_user).contentobject|attribute('has_online_contact_point').content.relation_list}
    {/if}
    <div class="table-responsive">
        <table class="table table-striped table-condensed{if $contacts|count()|eq(0)} hide{/if}" cellspacing="0">
        <thead>
        <tr>
            <th class="tight"></th>
        ...
        <tr class="buttons">
          <td colspan="4">
            <button class="btn btn-sm btn-danger ezobject-relation-remove-button {if $contacts|count()|eq(0)}hide{/if}" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]">
