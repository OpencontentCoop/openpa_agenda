var OpenpaAgendaWhereTreeFilter = {

    name: 'luogo_evento.tag_ids',

    container: '.widget[data-filter="where"] > ul',
    ...
    init: function (view, filter) {
        ...
        $.opendataTools.tagsTree('Luoghi evento', function (response) {
    ...
    buildQuery: function () {
        ...
        if (currentValues.length && jQuery.inArray( 'all', currentValues ) == -1) {
            return 'luogo_evento.tag_ids in [\'' + currentValues.join("','") + '\']';
        }
    },

    buildQueryFacet: function(){
        return 'luogo_evento.tag_ids|alpha|100';
    },
