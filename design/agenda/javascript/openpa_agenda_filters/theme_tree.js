var OpenpaAgendaThemeTreeFilter = {

    name: 'tematica_evento.tag_ids',

    container: '.widget[data-filter="theme"] > ul',
    ...
    renderTagTree: function (tag, container, filter) {
        var li = $('<li><a href="#" data-value="' + tag.id + '" data-name="' + tag.keyword + '">' + tag.keyword + '</a></li>').hide();
        if (tag.hasChildren) {
    ...
    init: function (view, filter) {
        ...
        $.opendataTools.tagsTree('Tematica evento', function (response) {
    ...
    buildQuery: function () {
        ...
        if (currentValues.length && jQuery.inArray( 'all', currentValues ) == -1) {
            return 'tematica_evento.tag_ids in [\'' + currentValues.join("','") + '\']';
        }
    },

    buildQueryFacet: function(){
        return 'tematica_evento.tag_ids|alpha|100';
    },
    ...
    refresh: function (response, view) {
        ...
        $.each(current, function () {
            if (this != 'all') {
                ...
                item.html(item.data('name')).parent().addClass('active').show().parents('li').show();
                currentLength++;
            }
        });
